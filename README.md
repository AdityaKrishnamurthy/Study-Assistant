...
Built with **Next.js (App Router)**, **TypeScript** data contracts, and **Tailwind CSS**, with a resilient multi-provider AI backend supporting OpenRouter (google/gemini-2.5-flash) as the main provider and direct Google Gemini API (gemini-2.0-flash) as fallback.
...
```
> **Note:** At least one valid API key is required. The server route uses a 2-tier automatic fallback chain: `OpenRouter (google/gemini-2.5-flash)` (Main) ➔ `Gemini Direct (gemini-2.0-flash)` (Fallback).
...
- **Resilient Fallback Architecture:** Designed with multi-provider fallback (OpenRouter with google/gemini-2.5-flash as main ➔ Gemini Direct gemini-2.0-flash as fallback) so client requests seamlessly failover if a primary provider hits rate limits or quota boundaries.
